...
** z2jh lacks (or lacked) features we required
*** Persistent storage requires autoprovisioned PVs -- not available at NCSA
*** No way to do shared persistent storage
*** KubeSpawner currently lacks user namespace support
*** Doesn't have a good way to do run-as-individual-user-uid
*** When we did our initial versions, it didn't support JupyterLab
...
***  It quietly became the assumed final shape of the Science Platform Interactive Notebook
Aspect.
*** Then people wanted to start using it for stuff like telescope control and commissioning
...
Phase 1 of this is largely done, as of Tuesday: ConfigMaps will be used
in preference to environmental variables in some big use-cases.

Phase 2 is removing the environmental variable injection, which should
wait until the old images have largely rolled off the spawner page (some
rework required).
...
want to build a Science Platform, though, you probably do.  So maybe
there are only 3 dozen sites that want this, but they're big and
important sites.  We would need to convince them they want it.

*** Semi-automate bleed-to-prod-candidate process

This has been made more complicated by conda being unable to solve
current stack environments, but I think we could write a small piece of
software to extract the packages from a running image and write a
requirements file for a test build.  That plus NotebookCI would give us
more confidence and shorter turnarounds for lifting prod packages.
...
~@lsst-sqre/jupyterlab-savequit~ and
~@lsst-sqre/jupyterlab-lsstextensions~ (note that no matter how awful
NPM is, at least it's namespaced).  Well, and Firefly is
first-and-a-half party.

*** JupyterLab 2.x Lab Extension Implementation

I would very much like it if someone *good* at TypeScript took over
these, but they are simple and will not be hard to port.  The first may
be done for us already.  That person would have more, or at least
